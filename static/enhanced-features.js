...
/**
 * Format AI responses with markdown support (bold, headings, bullet lists, numbered lists)
 * Fully compatible with Telugu Unicode text and English.
 */
window.formatAiResponse = function(text) {
    ...
    function formatInline(str) {
        if (!str) return '';
        let s = window.escapeHtml(str);
        
        // Bold: **text** or __text__
        s = s.replace(/\*\*(.+?)\*\*/g, '<strong class="chat-bold">$1</strong>');
        s = s.replace(/__(.+?)__/g, '<strong class="chat-bold">$1</strong>');
        
        // Italic: *text* or _text_
        s = s.replace(/(^|[^\*])\*([^\*]+?)\*([^\*]|$)/g, '$1<em class="chat-em">$2</em>$3');
        s = s.replace(/(^|[^_])_([^_]+?)_([^_]|$)/g, '$1<em class="chat-em">$2</em>$3');
        
        // Inline code / tags: `text`
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (headingMatch) {
            closeList();
            const level = Math.min(headingMatch[1].length + 2, 5); // 3, 4, 5
            html += `<h${level} class="chat-heading">${formatInline(headingMatch[2])}</h${level}>`;
            continue;
        }
        ...
        if (bulletMatch) {
            if (!inList || listTag !== 'ul') {
                closeList();
                html += '<ul class="chat-list chat-bullet-list">';
                inList = true;
                listTag = 'ul';
            }
        ...
        if (numMatch) {
            if (!inList || listTag !== 'ol') {
                closeList();
                html += '<ol class="chat-list chat-ordered-list">';
                inList = true;
                listTag = 'ol';
            }
        ...
        // Regular paragraph / standalone text line
        closeList();
        html += `<p class="chat-para">${formatInline(line)}</p>`;
    }
...
function generateAndPlayDetailSpeech(isEn) {
...
function appendChatMessage(text, role) {
    ...
    if (role === 'bot' && window.formatAiResponse) {
        message.innerHTML = window.formatAiResponse(text);
    } else if (role === 'bot') {
        message.innerHTML = `<p class="chat-para">${window.escapeHtml(text).replace(/\n/g, '<br>')}</p>`;
    } else {
        const paragraph = document.createElement('p');
        paragraph.className = 'chat-para';
...
function closeChat() {
    ...
    if (previousFocusChat && typeof previousFocusChat.focus === 'function') {
...
async function sendChatQuestion(question) {
    ...
    try {
        ...
        loading.remove();
        const replyText = data.response || data.answer || '';
